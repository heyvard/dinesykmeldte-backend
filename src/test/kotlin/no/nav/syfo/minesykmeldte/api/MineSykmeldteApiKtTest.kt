...
object MineSykmeldteApiKtTest : Spek({
    ...
    with(TestApplicationEngine()) {
        ...
        describe("/api/sykmelding/{id}") {
            ...
            it("should respond with the correct content if found") {
                every {
                    ...
                    sykmeldingId = UUID.fromString("7eac0c9d-eb1e-4b5f-82e0-aa4961fd5657"),
        ...
        describe("/api/soknad/{id}") {
            it("should respond with bad request if missing soknadId is missing") {
                with(
                    handleRequest(HttpMethod.Get, "/api/soknad/not-a-uuid") {
                        addAuthorizationHeader()
                    }
                ) {
                    response.status() shouldBeEqualTo HttpStatusCode.BadRequest
                    response.content shouldBeEqualTo """{ "message": "Soknad ID is not a valid UUID" }""".minifyApiResponse()
                }
            }
            ...
            it("should respond with the correct content if found") {
                every {
                    mineSykmeldteService.getSoknad(
                        UUID.fromString("d9ca08ca-bdbf-4571-ba4f-109c3642047b"),
                        any()
                    )
                } returns createSoknadTestData(
                    soknadId = UUID.fromString("d9ca08ca-bdbf-4571-ba4f-109c3642047b"),
                    sykmeldingId = UUID.fromString("772e674d-0422-4a5e-b779-a8819abf5959"),
